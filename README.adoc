...
==== icon:attached[width="20"] The device shows attached but does not transmit data 
...
* a reset connectivity icon:reset_connectivity[width="20"] on Connected Devices can also reset the network state and allow your device to freshly reattach 
...
The device itself can configure a DNS service that it uses to resolve domain names. If the device does not use a DNS than EMnify will provide a domain name service to the device. As a default EMnify will route all DNS queries over googles public DNS `8.8.8.8`. 

Customers can also configure to use their own DNS - no matter if a public or private one.
The DNS setting can be changed in the Portal -> Device Policies -> Service Policies -> More Options -> DNS

image:dns_setting.png[].Custom DNS setting configuration


To utilize a private DNS server which is not reachable via a public IP, requires to setup a private network with the machine or network where the private DNS is located. This can be done using Cloud Connect either with Amazon Transit Gateway or IPsec. A tutorial on how to setup a DNS firewall based on a private DNS using Amazon Route 53 is available link:https://www.emnify.com/en/developer-hub/dns-filtering[here]
