...
The device will show different icons that indicate the status (`Offline`, `Attached`, `Online`, `Blocked`). If no icon is shown than assign a SIM to the device. 

For most troubleshooting a view on the Details->Events is necessary. 
...
. Update GPRS location - the SIM card has also succesfully registered for data sessions with a different network element. 
...
image:offline.png[width=20,float="left"] *The device is offline* 

* click on Details -> ensure that device is enabled
* click on Details -> events. Validate if there is any location update event created and rejected. The reasoning should indicate the resolution to fix the problem. If there is no location update event:
...
image:attached.png[width=20,float="left"] *The device shows attached but does not transmit data* 
...
image:online.png[width=20,float="left"] *The device shows online but does not transmit data**
...
* a reset connectivity image:reset_connectivity.png[width=20] on Connected Devices can also reset the network state and allow your device to freshly reattach 
...
IoT SIMs are more durable than regular SIM cards and come in different form factors and quality grades. EMnify has also a software application directly on the SIM so different operator profiles can be used based on the location of the device - so EMnify can provide more networks than traditional operators. 
...
EMnify offers an easy entry to test the services and platform by downloading an EMnify eSIM profile to an eSIM capable phone or tablet. During the trial period every organisation has the option to download one profile which can be used instantly. 

The eSIM does not use a multi-IMSI applet (as on the physical SIM cards) and therefore has some difference in the network coverage. For a list of supported devices and limitations please refer to the link:https://support.emnify.com/hc/en-us/articles/360021545600-Evaluation-eSIM-device-support-and-restrictions#h_01F7V2BVKT0RQRVXK3VNSPYQMW[knowledge base] 

The eSIM can be used to test and verify all EMnify functionality including:
